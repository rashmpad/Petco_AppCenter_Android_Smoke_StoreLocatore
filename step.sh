...
echo "Rashmi + ${app_path}"
#npm install appcenter-cli@1.0.8 -g
#appcenter test generate uitest --platform android --output-path "${OUTPUT_PATH}"
#nuget restore -NonInteractive "${SOLUTION}"
#msbuild "${SOLUTION}" /p:Configuration=Release
appcenter test prepare uitest --artifacts-dir "${ARTIFACTS_DIR}" --app-path "${app_path}" --build-dir "${BUILD_DIR}" --fixture "Petco.UITests.AM_ProfileLanding(Android).VerifyProfileLanding" --fixture "Petco.UITests.Cart(Android).VerifyCartFlowSecureCheckOutForgotPasswordwithOutRepeatDelivery" --debug --quiet
